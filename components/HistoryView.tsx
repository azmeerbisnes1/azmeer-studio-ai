  ...
  const fetchHistory = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      if (Array.isArray(items)) {
        ...
        // For items that are completed but missing a URL (often the list doesn't have it),
        // we might need to fetch the specific history if it's the latest one being viewed.
        // But the VideoCard handles error retries, so let's keep it simple here.

        // SYNC STRATEGY: Poll if there are items in progress (status 1)
      ...
      setError(err.message || "Gagal menyelaraskan arkib neural.");
    } finally {
      ...
      <div className="max-w-7xl mx-auto w-full">
        <header className="mb-20 flex flex-col lg:flex-row lg:items-end justify-between gap-10 pt-10 animate-up">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-cyan-500 text-[10px] font-black uppercase tracking-[0.6em]">neural vault archive</p>
            </div>
          ...
          >
            ...
            {loading ? 'MENYELARAS...' : 'REFRESH ARKIB'}
          </button>
        </header>
        ...
        {error && (
          <div className="bg-red-500/5 border-2 border-red-500/20 p-10 rounded-[3rem] mb-16 flex flex-col gap-6 animate-up">
            <div className="flex items-center gap-4">
               ...
               <p className="text-red-500 text-xs font-black uppercase tracking-[0.3em]">Network Error Detected</p>
            </div>
            <p className="text-red-400/60 text-[11px] font-mono leading-relaxed bg-black/40 p-6 rounded-2xl border border-red-500/10">
              {error}
            </p>
            <button onClick={() => fetchHistory(true)} className="w-fit px-8 py-3 bg-red-600/20 hover:bg-red-600 text-white text-[10px] font-black uppercase rounded-xl transition-all">Retry Secure Tunnel</button>
          </div>
        )}
        ...
        {history.length === 0 && !loading && !error ? (
          <div className="text-center py-48 border-4 border-dashed border-slate-900/50 rounded-[4rem] bg-slate-900/5 animate-up">
            ...
            <p className="text-slate-700 font-black uppercase tracking-[0.5em] text-xs">Arkib Cloud Kosong</p>
          </div>
        ) : (
