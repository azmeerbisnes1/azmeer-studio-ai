...
export const VideoCard: React.FC<{ video: GeneratedVideo }> = ({ video }) => {
  ...
  // Auto-establish link apabila video siap
  useEffect(() => {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      // Guna blob src sedia ada atau fetch baru
      ...
      <div className="relative aspect-video bg-black flex items-center justify-center overflow-hidden">
        {isCompleted && !videoError ? (
          <>
            ...
            {isLoadingNeural && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 z-20">
                 ...
                 <p className="text-[8px] font-black text-cyan-400 uppercase tracking-widest animate-pulse">Syncing Video Link...</p>
              </div>
            )}
          ...
          <div className="text-center p-8 space-y-4">
             ...
             <button onClick={(e) => { e.stopPropagation(); establishNeuralLink(); }} className="text-[8px] text-slate-500 hover:text-white uppercase tracking-widest underline underline-offset-4 decoration-slate-700">Cuba Sync Lagi</button>
          </div>
        )}
      </div>
      ...
      <div className="p-8 flex-grow flex flex-col">
        ...
        <div className="mt-auto pt-6 border-t border-white/5">
          ...
          >
            ...
            <span>{isDownloading ? 'EXTRACTING...' : 'DOWNLOAD VIDEO'}</span>
          </button>
        </div>
      </div>
    </div>
