...
const ImageLabView: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("Gagal buat gambar. Sila cuba lagi.");
    } finally {
      ...
      <div className="max-w-6xl mx-auto w-full pb-32">
        <header className="mb-16 text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/30">
            <span className="text-[10px] font-black text-orange-400 uppercase tracking-widest">Bilik Gambar AI</span>
          </div>
          <h2 className="text-6xl md:text-8xl font-black text-white tracking-tighter uppercase leading-none">
            Makmal <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-rose-500">Imej</span>
          </h2>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-5 space-y-8">
            <div className="glass-panel p-8 rounded-[3rem] border border-white/5">
              <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Ceritakan Gambar Anda</label>
              <textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Tuliskan imaginasi visual anda di sini..."
              ...
              <div className="mt-10">
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6">Format Gambar</label>
              ...
              >
                {isGenerating ? (
                  <>
                    ...
                    <span>Tengah Proses...</span>
                  </>
                ) : (
                  <>
                    <span>Jana Gambar</span>
              ...
              {result ? (
                ...
                <div className="text-center space-y-8 p-12">
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-black text-orange-400 uppercase tracking-widest animate-pulse">Menyiapkan Visual</p>
                    <p className="text-[10px] text-slate-600 uppercase tracking-[0.3em]">Memproses Data Imej</p>
                  </div>
                </div>
              ) : (
                <div className="text-center space-y-10 px-16">
                  ...
                  <p className="text-slate-700 text-[10px] font-black uppercase tracking-[0.4em] leading-relaxed">Gambar anda akan muncul di sini.</p>
                </div>
              )}
            </div>
          </div>
