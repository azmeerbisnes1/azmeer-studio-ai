...
const HistoryView: React.FC<HistoryViewProps> = ({ user }) => {
  ...
  const fetchHistory = useCallback(async (showLoading = true) => {
    ...
    try {
      // 1. Ambil semua UUID daripada pengkalan data user (Supabase)
      ...
      // 2. Dapatkan data terperinci secara live daripada Geminigen untuk setiap UUID
      ...
      // 3. Tapis data yang berjaya dan map ke format UI
      const videoItems = rawResults
        .filter(item => item !== null && (item.data || item.uuid || item.id))
        .map(item => mapToGeneratedVideo(item))
        .sort((a, b) => b.timestamp - a.timestamp);
          
      setHistory(videoItems);

      // 4. Polling Automatik: Jika ada video yang masih berstatus 'Processing' (Status 1)
      ...
      setError(err.message || "Gagal menyelaraskan Arkib Video.");
    } finally {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#020617] p-6 md:p-12 overflow-y-auto custom-scrollbar">
      <div className="max-w-7xl mx-auto w-full">
        <header className="mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-8 pt-6 animate-up">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <p className="text-cyan-500 text-[10px] font-black uppercase tracking-[0.5em]">Neural Sync Node</p>
            </div>
            <h2 className="text-5xl md:text-7xl font-black text-white tracking-tighter uppercase leading-none">
              Vault <span className="text-slate-900 stroke-text">History</span>
            </h2>
          </div>
          <button 
        ...
        {error && (
          <div className="mb-10 p-6 bg-red-500/10 border border-red-500/20 rounded-3xl text-center max-w-2xl mx-auto animate-up">
             <p className="text-red-400 text-xs font-bold uppercase tracking-widest mb-2">Sync Connection Error</p>
             <p className="text-red-400/70 text-[10px] font-medium leading-relaxed italic">"{error}"</p>
             <button onClick={() => fetchHistory(true)} className="mt-4 text-[9px] text-white/50 hover:text-white uppercase tracking-widest underline decoration-dashed">Try Reconnect</button>
          </div>
        )}
        ...
        {history.length === 0 && !loading ? (
          <div className="text-center py-40 border-4 border-dashed border-slate-900/50 rounded-[4rem] bg-slate-900/5 animate-up">
            ...
            <p className="text-slate-700 font-black uppercase tracking-[0.4em] text-[10px]">Tiada rekod video ditemui dalam arkib anda.</p>
          </div>
        ) : (
